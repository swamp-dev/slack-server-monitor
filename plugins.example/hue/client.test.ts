...
describe('hue client', () => {
  ...
  describe('getConfig', () => {
    it('should return config when env vars are set', async () => {
      const { getConfig } = await import('./client.js');
    ...
    it('should throw HueNotConfiguredError when BRIDGE_IP missing', async () => {
      ...
      const { getConfig } = await import('./client.js');
      const { HueNotConfiguredError } = await import('./types.js');
  ...
  describe('isTransientError', () => {
    it('should return true for HueApiError with 503', async () => {
      const { isTransientError } = await import('./client.js');
      const { HueApiError } = await import('./types.js');
    ...
    it('should return true for HueBridgeUnreachableError', async () => {
      const { isTransientError } = await import('./client.js');
      const { HueBridgeUnreachableError } = await import('./types.js');
