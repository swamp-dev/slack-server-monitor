...
const conversationDetailStyles = `
  ...
  .conv-header-top {
    ...
    gap: 12px;
    /* Stays on one row at desktop — the title group ellipsis-truncates
       instead of pushing the toolbar to a new line. The mobile media
       query below re-enables wrap. */
    flex-wrap: nowrap;
  }
  /* The title group (back link + h1) needs to shrink with ellipsis so a
     long title doesn't push the toolbar onto its own row. min-width: 0
     lets the flex item collapse below content size; the title itself
     handles the ellipsis. */
  .conv-title-group {
    display: flex;
    align-items: center;
    gap: 8px;
  ...
  @media (max-width: 640px) {
    .conv-header-top { flex-wrap: wrap; }
