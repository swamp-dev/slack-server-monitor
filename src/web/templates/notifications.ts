...
export function renderNotificationDropdown(notifications: Notification[]): string {
  ...
  const items = groups.map((g) => {
    ...
    // Error-level items get a one-line body preview so the dropdown
    // surfaces what's wrong without forcing a click-through. The preview
    // span sits after notif-time in DOM order so the grid lays out as
    // [title | time] on row 1 and [preview spanning] on row 2 — DOM
    // order matters because the preview spans both grid columns.
    const previewHtml = n.level === 'error' && n.body
      ? `<span class="notif-preview">${escapeHtml(n.body)}</span>`
      : '';
...
const notificationPageStyles = `
  ...
  .notif-entry .notif-entry-body {
    ...
    color: var(--text-muted);
    margin-top: 4px;
    /* Cap the body preview at 2 lines so a stack of notification cards
       keeps an even vertical rhythm. The full body is still readable
       on the linked page or by hover-expanding (future). */
    display: -webkit-box;
    -webkit-line-clamp: 2;
