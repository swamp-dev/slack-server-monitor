...
/**
 * Resolve the agentbox binary path with the following precedence:
 *   1. AGENTBOX_BINARY_PATH env var (explicit override)
 *   2. <repo-root>/dist/bin/agentbox (default — produced by `npm run agentbox:build`
 *      or the Dockerfile's agentbox-builder stage)
 *
 * Computed once at module load. The repo root is derived module-relative so it
 * works from both the tsc-built dist/utils/shell.js and the tsx-loaded
 * src/utils/shell.ts (both are two levels below the repo root).
 */
function resolveAgentboxPath(): string {
  const fromEnv = process.env.AGENTBOX_BINARY_PATH;
  if (fromEnv && fromEnv.trim() !== '') return fromEnv;
  const repoRoot = path.resolve(fileURLToPath(new URL('../..', import.meta.url)));
  return path.join(repoRoot, 'dist', 'bin', 'agentbox');
}

const AGENTBOX_BINARY_PATH = resolveAgentboxPath();
...
const ALLOWED_COMMANDS = new Map<string, string>([
  ...
  // GitHub CLI
  ['gh', '/usr/bin/gh'],
  // AgentBox automated-run CLI — path resolved at module load (env override
  // or default to the binary produced by the bundled vendor/agentbox submodule).
  ['agentbox', AGENTBOX_BINARY_PATH],
]);
