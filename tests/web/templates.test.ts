...
describe('web templates', () => {
  ...
  describe('dashboard home page', () => {
    ...
    describe('uptime card severity', () => {
      // Issue #380: when other health metrics are CRITICAL, the uptime card
      // shouldn't keep showing a green ONLINE pill — that's a semantic
      // mismatch that confuses an at-a-glance read of the dashboard.
      const healthyHealth: ServerHealth = {
        uptime: '12 days, 4:32',
        uptimeSeconds: 12 * 86400,
        loadAverage: [0.42, 0.38, 0.31],
        cpu: { cores: 4, model: 'test' },
        memory: { total: 16384, used: 8000, free: 4000, available: 8000, bufferCache: 4000, percentUsed: 49 },
        swap: { total: 4096, used: 0, free: 4096, percentUsed: 0 },
        disks: [{ filesystem: '/dev/sda1', size: '100G', used: '40G', available: '60G', percentUsed: 40, mountPoint: '/' }],
        timestamp: Date.now(),
      };
      const criticalHealth: ServerHealth = {
        ...healthyHealth,
        loadAverage: [6.8, 5.2, 4.1],
        memory: { total: 16384, used: 15200, free: 384, available: 1184, bufferCache: 800, percentUsed: 93 },
        disks: [{ filesystem: '/dev/sda1', size: '100G', used: '92G', available: '8G', percentUsed: 92, mountPoint: '/' }],
      };
      const warnHealth: ServerHealth = {
        ...healthyHealth,
        memory: { total: 16384, used: 13000, free: 1500, available: 3000, bufferCache: 1500, percentUsed: 80 },
      };

      function uptimeStatusBlock(html: string): string {
        // Pull out the contents of the first health-card div (the uptime
        // card is rendered first in the grid).
        const m = /<div class="health-card[^"]*">[\s\S]*?<div class="health-status-text [^"]+">[^<]*<\/div>/m.exec(html);
        return m?.[0] ?? '';
      }

      it('shows Online with ok class when all sibling metrics are healthy', () => {
        const html = renderDashboard(baseStats, recentSessions, favoriteSessions, 1, allTags, 'U01TEST', undefined, undefined, undefined, healthyHealth);
        const block = uptimeStatusBlock(html);
        expect(block).toContain('health-status-text ok');
        expect(block).toContain('Online');
      });

      it('escalates to warn class when a sibling metric is degraded', () => {
        const html = renderDashboard(baseStats, recentSessions, favoriteSessions, 1, allTags, 'U01TEST', undefined, undefined, undefined, warnHealth);
        const block = uptimeStatusBlock(html);
        expect(block).toContain('health-status-text warn');
        expect(block).not.toContain('health-status-text ok');
      });

      it('escalates to danger class when a sibling metric is critical', () => {
        const html = renderDashboard(baseStats, recentSessions, favoriteSessions, 1, allTags, 'U01TEST', undefined, undefined, undefined, criticalHealth);
        const block = uptimeStatusBlock(html);
        expect(block).toContain('health-status-text danger');
        expect(block).not.toContain('Online');
      });
    });
  });
