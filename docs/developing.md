...
[AgentBox](https://github.com/swamp-dev/agentbox) runs AI agents in isolated Docker containers for implementing features via PRD-driven ralph loops. **The agentbox source ships as a git submodule at `vendor/agentbox/` and the binary builds as part of the project.**

### Prerequisites

After cloning, pull the submodule and build the binary:

```bash
git submodule update --init --recursive   # fetches vendor/agentbox source
npm run agentbox:build                     # produces dist/bin/agentbox (requires Go 1.24+)
```

The Dockerfile builds the binary automatically — production deploys don't need the manual step. If Go isn't installed locally, `npm run agentbox:build` skips with a friendly note; you can either install Go or rely on the Docker build.

The plugin reads `AGENTBOX_BINARY_PATH` from the environment if set; default is `<repo-root>/dist/bin/agentbox`.
...
The plugin loader scans both `plugins.example/` (bundled, ships in the repo) and `plugins.local/` (gitignored, your local overrides). When the same filename appears in both, **`plugins.local/` wins**.

This means you can fork-and-patch a bundled plugin by copying it to `plugins.local/` without touching the committed copy. Useful for short-term hacks. For changes you want to keep, edit the bundled file directly via a PR.

### Gitignore Warning

`plugins.local/` is still gitignored. Ralph's auto-commit will NOT include gitignored files; if you're using ralph to iterate on a plugin you want to keep, work on the bundled copy in `plugins.example/` directly.
