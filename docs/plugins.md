...
- `plugins.example/` — bundled plugins that ship with the repo (e.g. `agentbox`). Loaded by default on every fresh clone.
- `plugins.local/` — your local overrides (gitignored). A file with the same basename as a bundled plugin replaces it; the local copy wins on conflict.

```bash
# Use the bundled plugins as-is
npm run dev

# Or override a bundled plugin locally without forking
mkdir -p plugins.local
cp plugins.example/agentbox.ts plugins.local/agentbox.ts
# edit plugins.local/agentbox.ts to taste; restart
```

Plugins are `.ts` or `.js` files at the top level of either directory, with a default export implementing the `Plugin` interface. They are loaded at startup via [jiti](https://github.com/unjs/jiti) — no build step required.
